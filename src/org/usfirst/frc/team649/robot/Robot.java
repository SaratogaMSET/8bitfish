...
//		}).start();
	...
	public void teleopPeriodic() {
		...
		// }
		if (oi.operatorJoystick.getRawButton(2)) {
			// new GyroStraightPID(282000, -50, 180000).start();

		} else {
			// drive.resetEncoders();
			drive.driveFwdRotate(oi.driver.getForward(), -oi.driver.getRotation(), true);
		}
		SmartDashboard.putNumber("DT Left Talon Raw", drive.motors[0].getSelectedSensorPosition(0));
		SmartDashboard.putNumber("DT Right Talon Raw", drive.motors[0].getSelectedSensorPosition(0));
	...
	{
		char[] CharArray = WriteString.toCharArray(); // Create an array of characters. This breaks up the information
														// into something that can be passed over the I2C bus.
		byte[] RobotStatus = new byte[CharArray.length]; // Characters cannot be passed over I2C, thus we must convert
															// them to bytes. This line creates the byte array.
		for (int i = 0; i < CharArray.length; i++)// Create a loop that fills the new byte array. The new byte array is
													// the same size as the character array.
		{
			RobotStatus[i] = (byte) CharArray[i]; // Pass information slot by slot. This also converts the characters
													// into bytes.
		}
		// arduino.transaction(RobotStatus, RobotStatus.length, null, 0); //One type of
		// sending info over the I2C bus. This method asks for a response from the
		// receiving unit. Caused null point exceptions.
		arduino.writeBulk(RobotStatus, RobotStatus.length); // This method sends info one way, without demanding a
															// response from reader unit.
	}
